...
request_queue_url = f'https://sqs.us-east-1.amazonaws.com/442042549532/{ASU_ID}-req-queue'
response_queue_url = f'https://sqs.us-east-1.amazonaws.com/442042549532/{ASU_ID}-resp-queue'
s3 = boto3.client('s3')
...
def process_image(image_data):
    ...
    print(f"Face recognition output: {process.stdout}")
    if process.returncode == 0:
        return process.stdout.strip()
    else:
        print(f"Error in face recognition: {process.stderr}")
    ...
    while True:
        ...
        if 'Messages' in response:
            for message in response['Messages']:
                ...
                file_name = body['imageKey']
                file_content = body['imageContent'] 
